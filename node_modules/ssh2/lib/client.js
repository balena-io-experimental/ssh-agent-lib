...
Client.prototype.connect = function(cfg) {
  ...
  this.config.remoteAuth = (typeof cfg.remoteAuth === 'boolean'
                              ? cfg. remoteAuth
                              : false);
  ...
  stream.on('drain', function() {
    ...
    sock.resume();
  }).on('fingerprint', function(info, cb) {
    self.emit('fingerprint', info);
    cb(true);
  });
  ...
  if (this.config.remoteAuth)
    auths.push('remoteauth');
  ...
  function tryNextAuth() {
    ...
    switch (curAuth) {
      ...
      break;
      case 'remoteauth':
        self.emit('remoteauth', self.config.username, stream);
        break;
      case 'publickey':
